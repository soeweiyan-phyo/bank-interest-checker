...
const scrapeCurrWebData = async () => {
  const browser = await puppeteer.launch({ headless: false });
  const page = await browser.newPage();
  await page.goto(url);

  const scrapedTablesData = await page.evaluate(() => {
    // Helper function
  ...
  // console.log(scrapedTablesData);
  await browser.close();

  return scrapedTablesData;
};

module.exports = scrapeCurrWebData;
